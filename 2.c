...
static const double pos[][2] = {
    {0, 0}, {2.5, 5.0 / 6}, {5.0, 10.0 / 6}, {7.5, 15.0 / 6}};
...
event movies(i += 50; t <= 100) {
  ...
  output_ppm(omega, file = path, box = {{-d, -d}, {d, d}}, min = -200,
             max = 200, linear = false, mask = m);
  ...
  output_ppm(omega, file = path, box = {{-0.5, -0.45}, {3.5, 0.45}}, min = -200,
             max = 200, linear = false, mask = m);
  iframe++;
}
event adapt(i++) {
